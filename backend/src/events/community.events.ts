...
communityEvents.on(
    ...
    async (payload: MessageCreatedPayload) => {
        const { messageId, communityId, userId, content, addictionType } =
            payload;
        ...
        const user = await prisma.user.findUnique({
            ...
            include: {
                addictions: {
                    where: { type: addictionType, status: 'ACTIVE' },
                    take: 1,
                },
            },
        });
        ...
        if (user?.addictions[0]) {
            const streak = await streakService.computeStreak(
                userId,
                user.addictions[0].id,
            );
...
communityEvents.on(
    'message:flagged',
    async (payload: MessageFlaggedPayload) => {
