  ...
  // Fetch full user context for prompt injection
    ...
    async generate(prompt: string, cacheKey?: string, ttl = 300): Promise<string> {
        ...
        try {
            ...
            console.warn('[AI] Gemini failed, falling back to HuggingFace:', geminiError);
        try {
    ...
    async coach(userId: string, userMessage: string, recentMessages: { role: 'user' | 'assistant'; content: string }[]): Promise<string> {
        const userContext = await aiService.buildUserContext(userId);
        const prompt = promptBuilder.coach({ userContext, userMessage, recentMessages });
        return aiService.generate(prompt, undefined, CACHE_TTL.coach);
    },

    async urge(userId: string, trigger: string, mood: string, intensity: number): Promise<string> {
        const userContext = await aiService.buildUserContext(userId);
        const prompt = promptBuilder.urge({ userContext, trigger, mood, intensity });
    ...
    async insight(userId: string): Promise<string> {
        ...
        const topTriggers = Object.entries(triggerFreq).sort((a, b) => b[1] - a[1]).slice(0, 3) as [string, number][];
        const topMoods = Object.entries(moodFreq).sort((a, b) => b[1] - a[1]).slice(0, 3) as [string, number][];
    ...
    async saveMessage(userId: string, role: 'user' | 'assistant', content: string, type: 'COACH' | 'URGE' | 'INSIGHT', context?: object): Promise<void> {
