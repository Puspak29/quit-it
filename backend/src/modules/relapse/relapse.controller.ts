...
export const relapseController = {
    async log(req: AuthRequest, res: Response): Promise<void> {
        const { addictionId, trigger, mood, intensity, note, context } =
            req.body;
    ...
    async getAll(req: AuthRequest, res: Response): Promise<void> {
        ...
        sendSuccess(res, HTTP_STATUS.OK, 'Relapses retrieved', {
            relapses,
            total,
            page: Number(page),
            limit: Number(limit),
    ...
    async getPatterns(req: AuthRequest, res: Response): Promise<void> {
        ...
        if (cached) {
            sendSuccess(res, HTTP_STATUS.OK, 'Patterns retrieved', {
                patterns: cached,
            });
            return;
        }
